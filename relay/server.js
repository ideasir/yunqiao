...
function createMcpServer(userId) {
  ...
  server.registerTool('get_client_messages', {
    description: '获取客户端发来的消息',
    inputSchema: z.object({}),
  }, async () => {
    const msgs = agentMessages.splice(0);
    if (msgs.length === 0) return { content: [{ type: 'text', text: '没有新消息' }] };
    const text = msgs.map(m => `[${new Date(m.time).toLocaleTimeString()}] ${m.deviceName}: ${m.text}`).join('\n');
    return { content: [{ type: 'text', text }] };
  });
...
wss.on('connection', (ws, req, user) => {
  ...
  ws.on('message', (raw) => {
    ...
    if (type === 'agent_message') {
      const device = devices.get(deviceId);
      if (device) {
        agentMessages.push({ id: randomUUID().slice(0, 8), text: msg.text || '', deviceName: device.name, time: new Date().toISOString() });
