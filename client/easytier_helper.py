...
def build_node_command(mesh_config):
    ...
    冲突规避设计（主任要求）：
      ...
      - TUN 模式：创建虚拟网卡，组网双向通信
      - 固定 IP 10.10.10.88/24
      - 监听 11010 端口接受组网内连接
    """
    cmd = [
        str(_core_path()),
        ...
        "-i", "10.10.10.88/24",                # 固定 IP，不 DHCP
        "--config-dir", str(_cache_dir()),       # 独立配置目录
        "-l", "tcp://0.0.0.0:11010",           # 监听 TCP 端口
        "-l", "udp://0.0.0.0:11010",           # 监听 UDP 端口
    ]
